<?php
include '../php_plus_mysql/mysql_proc_conn.php';
?>
...
<html>
    ...
    <div>
        ...
        <div class="pitanje">6. Napišite program koji s pomoću SQL funkcije COUNT prebrojava nastavnike 
            čije prezime počinje slovom M i ispisuje taj broj.</div>

        <div class="odgovor">

            <?php
            $query = "SELECT COUNT(nastavnik.prezNastavnik) AS broj_nastavnika
                        FROM nastavnik
                        WHERE
                        nastavnik.prezNastavnik RLIKE '^[mM].*';";
            ...
            while ($row = mysqli_fetch_array($result)) {
                //print_r($row);
                printf("Ukupan broj nastavnika kojima prezime počinje na M je: %d  <br>"
                        , $row['broj_nastavnika']
                );
            }
            echo '<hr>';
            ...
            <div>
                <?php if (!isset($_POST['broj_btn'])) { ?> 
                    <form method="POST" action="6zadatak.php">
                        Želite li pogledati koji su to profesori? 
                        <input type="submit" name="broj_btn" value="DA">
                    </form> 
                <?php } ?>          
                <?php
                if (isset($_POST['broj_btn'])) {
                    echo "Popis profesora";

                    $query2 = "SELECT nastavnik.prezNastavnik AS 'Prezime nastavnika', nastavnik.imeNastavnik AS 'Ime nastavnika'
                            FROM nastavnik
                            WHERE
                            nastavnik.prezNastavnik RLIKE '^[mM].*' ORDER BY prezNastavnik;";
